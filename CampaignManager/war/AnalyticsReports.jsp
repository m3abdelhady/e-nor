...
if(gaViewId == null || gaViewId==""){
	response.sendRedirect("/AnalyticsSettings.jsp?aid="+longAccountId);
}
%>
...
/* table.dataTable tbody tr{border:solid 1px #ccc; background:#fff !important;} */
.gapi-analytics-data-chart-styles-table-tr, .gapi-analytics-data-chart .gapi-analytics-data-chart-styles-table-tr-odd{background:#fff !important;}
</style> 
...
$('.DateRangeSelector-item input').daterangepicker();
...
gapi.analytics.ready(function() {
	gapi.analytics.auth.on('needsAuthorization', function() {
		   window.location.href = "/AnalyticsSettings.jsp?aid=<%=longAccountId%>";
  ...
  dataChart1.on('success', function(response) {
	  ...
	  $('.google-visualization-table-table tbody tr td:first-child').each(function() {	    	
			 var $this = $(this);
			 $this.before("<td style='text-align:center !important'></td>");
	...
	$('.google-visualization-table-table tr td:nth-child(9)').each(function() {	    	
	  ...
	  $('.google-visualization-table-table tr td:nth-child(2)').each(function() {
		    var $this = $(this);		    
		    $this.html("<a href='AnalyzSourceMedium.jsp?cname="+encodeURIComponent($this.text().replace('"', '\\\"'))+"&aid=<%=longAccountId%>'>"+$this.text()+"</a>");
...
var end = moment();
*/	  
/* var customDateRange = $('.DateRangeSelector-item input').daterangepicker(
		  {
...
/* customDateRange.on('change', function(data) {	
	
    dataChart1.set({query: data}).execute();	   
    
  });  */
	  
dateRangeSelector1.on('error', function(data) {	
