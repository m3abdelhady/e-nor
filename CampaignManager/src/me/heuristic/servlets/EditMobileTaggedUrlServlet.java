...
public class EditMobileTaggedUrlServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (errors.size()>0) {
    		...
    		returnUrl.append("/MCTaggedURLs.jsp?aid=" + longAccountId + "&cid=" + longCampaignId);
	    	if (Messages.ACTION_DELETE.equalsIgnoreCase(act)) {
	        	// DELETE
	    		Entity myTaggedUrl = MCTaggedUrlService.getSingleTaggedUrl(longAccountId, longCampaignId, longTaggedUrlId);
	    		MCTaggedUrlService.deleteTaggedUrl(myTaggedUrl);
	    		...
	    		if (errors.size()>0) {
	    			...
	    			for(int i=0; i< sTaggedUrlIds.length; i++){
	    			   // myCampaign = CampaignService.getSingleCampaign(longAccountId, Long.parseLong(sTaggedUrlIds[i]));
	    			    myTaggedUrl = MCTaggedUrlService.getSingleTaggedUrl(longAccountId, longCampaignId, Long.parseLong(sTaggedUrlIds[i]));
	    			    MCTaggedUrlService.deleteTaggedUrl(myTaggedUrl);  
		    	...
		    	if (errors.size()>0) {
		    		returnUrl.append("&tid=" +longTaggedUrlId + "&utm_url=" + utm_url +
		    				"&utm_term=" + utm_term + "&utm_content=" + utm_content + "&utm_extra=" + utm_extra);
		    		for (String error:errors) {
		    			returnUrl.append("&err=" + error);
		    		}
		    	} else {
				    ...
				    // save
		    		MCTaggedUrlService.createOrUpdateTaggedUrl(myEmail, longAccountId, longCampaignId, longTaggedUrlId,
		    				utm_term, utm_content, utm_extra);
