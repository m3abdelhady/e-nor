...
public class CampaignHolderService {
	...
	public static void deleteSelectedCampaignHolder(String[] campaignName, long longAccountId) {
	...
	/*public static void copyCampaignHolder(String campaignName, long longAccountId) {

		Iterable<Entity> campaigns = CampaignService.getAllCampaignsFor(longAccountId);
    	for (Entity campaign : campaigns) {	
    		if (campaign.getProperty("utm_campaign").toString().contentEquals(campaignName)){
    			
    			CampaignService.createOrUpdateCampaign(myEmail, longAccountId, 0, campaignName, campaign.getProperty("utm_meduim"), campaign.getProperty("utm_source"), campaign.getProperty("utm_notes"));
    			
    			Key campaignKey = campaign.getKey();
    			
    			campaign.setProperty("utm_campaign", campaignName);
    		    ...
    		    try {
    				...
    				PMUtil.persistEntity(taggedUrlKeys);
    				PMUtil.deleteEntity(campaignKey);
    		    } catch (Exception ex) {
    		    	// TODO
    		    	// String msg = Util.getErrorResponse(e);
	...
	public static Entity createOrUpdateCampaignHolder(String myEmail, long longAccountId, long longCampaignId,
		...
		if (0==longCampaignId) {
			...
			campaign = getSingleCampaignHolder(longAccountId, longCampaignId);
    	}
	...
	public static void UpdateCampaignHolder(String myEmail, long longAccountId, String old_utm_campaign ,String utm_campaign) {
       ...
       // Entity campaign;
		//Key key = KeyFactory.createKey("Account", longAccountId);
			...
			// editing existing campaign
    		// retrieve it from datastore
		Iterable<Entity> campaignHolders = getAllCampaignHoldersFor(longAccountId);
	...
	public static void upCampaignHolder(String campaignName, String utm_campaign,long longAccountId) {
	...
	public static int getAllCampaignHoldersCountFor(long longAccountId) {
	...
	public static int getAllCampaignHoldersCountForEmail(String myEmail) {
		 int pageSize = 1000;
		 int i = 0;
	...
	public static Iterable<Entity> getAllCampaignHoldersFor(long longAccountId) {
	    Key ancestorKey = KeyFactory.createKey("Account", longAccountId);
	...
	public static String renderCampaignHolderForm(Entity account,	long longCampaignId, String utm_campaign ) {
	    ...
	    sb.append("<form class=\"form\" action=\"/editcampaignholder\" method=\"post\" name=\"campaignForm\" onsubmit=\"return validateCampaignForm();\">\r\n");
...
*/
		...
		// sb.append("<a class=\"btn sbold green_default\" href='ManageCampaigns.jsp?aid="+longAccountId+"&cname="+utm_campaign+"&cid="+longCampaignId+"'>Cancel</a>");
		 sb.append("<a  class=\"btn default\" href='ManageCampaignHolders.jsp?aid="+longAccountId+"'>Cancel</a>");
	...
	public static String renderCampaignHolderDetails(String myRole, Entity campaign) {
	    StringBuilder sb = new StringBuilder();
	...
	public static String renderCampaignHolderList(String myRole, long longAccountId) {
	    StringBuilder sb = new StringBuilder();
		...
		if (UserType.hasPermission(myRole, UserType.PERMISSION_MANAGE_CAMPAIGNS)) {
			...
			sb.append("\t\t<input type=\"button\" value=\"+ New Campaign \" class=\"btn sbold green\" onClick=\"location.href='/EditCampaign.jsp?aid=" + longAccountId + "'\"/>\r\n");
		...
		sb.append("<form id='delSelected'  method='post' action='/editcampaignholder?aid="+longAccountId+"&act=" + Messages.ACTION_DELETEAll+"'>");
	//	=" + longAccountId  + "&act=" + Messages.ACTION_DELETEAll +"
		...
		for (Entity campaignHolder : campaignHolders) {		
			...
			sCampaignIds.add(longCampaignId);
			date = (Date) campaignHolder.getProperty("createdAt");
			sb.append("<tr>\r\n");
			sb.append("<td class='chekboxTD'><input type=\"checkbox\" name=\"chid\" class='checkboxes' value=\"" + campaignHolder.getProperty("utm_campaign").toString() + "\" /></td>\r\n");
			sb.append("<td><a href=\"/ManageCampaigns.jsp?aid=" + longAccountId + "&cname=" + campaignHolder.getProperty("utm_campaign") + "&cid=" + longCampaignId + "\">" + campaignHolder.getProperty("utm_campaign") + "</a></td>\r\n");
			...
			sb.append("<a href=\"/ManageCampaigns.jsp?aid=" + longAccountId + "&cname=" + campaignHolder.getProperty("utm_campaign") + "&cid=" + longCampaignId + "\">Details</a>");
			...
			if (UserType.hasPermission(myRole, UserType.PERMISSION_MANAGE_CAMPAIGNS) || UserType.hasPermission("ADMIN", UserType.PERMISSION_MANAGE_CAMPAIGNS)) {
				sb.append("&nbsp;|&nbsp;");
				sb.append("<a href=\"/EditCampaignHolder.jsp?aid=" + longAccountId + "&cid=" + longCampaignId +"&cname="+campaignHolder.getProperty("utm_campaign")+"&act=UpdateCHolder"+ "\">Edit</a>");
				sb.append("&nbsp;|&nbsp;");
				sb.append("<a id='copyCHolder' onclick=\"return confirm('Are you sure you want to copy campaign?')\" href=\"/editcampaignholder?aid=" + longAccountId + "&utm_campaign=" + campaignHolder.getProperty("utm_campaign") + "&utm_medium=" + campaignHolder.getProperty("utm_medium") + "&utm_source=" + campaignHolder.getProperty("utm_source") +"&act=" + Messages.ACTION_COPY+ "\">Copy</a>");
				sb.append("&nbsp;|&nbsp;");
				sb.append("<a href=\"javascript:confirmCampaignDelete(\'/editcampaignholder?aid=" + longAccountId  + "&utm_campaign=" + campaignHolder.getProperty("utm_campaign").toString().replace("'", "\\\'")+ "&act=" + Messages.ACTION_DELETE + "\');\">Delete</a>");
			...
			if(AnalyticsService.GetRefreshKey(longAccountId) !=null && AnalyticsService.GetViewId(longAccountId) !=null ){
				sb.append("&nbsp;|&nbsp;");
				sb.append("<a href=\"/AnalyticsCallBack?dis=AnalyzMediumSource&aid=" + longAccountId + "&cname=" + campaignHolder.getProperty("utm_campaign") + "&cid=" + longCampaignId + "\">Analyze <span style='padding-left: 2px; color: #dd7127; font-size: 9px; font-weight: bold; position: relative; text-transform: uppercase; top: -3px;'>BETA</span></a>");
			}
			sb.append("</td>\r\n");
			sb.append("</tr>\r\n");
			i++;
	...
	public static String renderCampaignHolderExportForm(long longAccountId) {
	    StringBuilder sb = new StringBuilder();

		// retrieve the campaigns belonging to the selected Account.
		Iterable<Entity> campaigns = getAllCampaignHoldersFor(longAccountId);
	...
	public static String renderCampaignHolderImportForm(long longAccountId) {
	    StringBuilder sb = new StringBuilder();
