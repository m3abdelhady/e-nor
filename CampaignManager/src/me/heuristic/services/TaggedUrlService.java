...
public class TaggedUrlService {
	public static void deleteTaggedUrl(Entity taggedUrl) {
    	Key taggedUrlKey = taggedUrl.getKey();
    	PMUtil.deleteEntity(taggedUrlKey);
	}
	...
	public static Entity createOrUpdateTaggedUrl(String myEmail,
		...
		if (0==longTaggedUrlId) {
			// new account
			taggedUrl = new Entity("TaggedUrl", key);
	        taggedUrl.setProperty("createdBy", myEmail);
	        taggedUrl.setProperty("createdAt", date);
    	} else {
			// editing existing taggedUrl
			taggedUrl = getSingleTaggedUrl(longAccountId, longCampaignId, longTaggedUrlId);
    	}

        // setting properties
    	taggedUrl.setProperty("utm_url", utm_url);
    	taggedUrl.setProperty("utm_term", utm_term);
    	taggedUrl.setProperty("utm_content", utm_content);
    	taggedUrl.setProperty("utm_extra", utm_extra);
        taggedUrl.setProperty("modifiedBy", myEmail);
        taggedUrl.setProperty("modifiedAt", date);
	...
	public static String renderTaggedUrlShareForm(String shortUrl, String completeUrl) {
		...
		sb.append("<a href=\"http://www.facebook.com/sharer.php?u=" + HtmlUtil.encode(completeUrl) + "&t=" + HtmlUtil.encode("") +
    	...
    	sb.append("\t<a target=\"_blank\" name=\"twt_share\" href=\"https://twitter.com/share?url=" + HtmlUtil.encode(completeUrl) +
    	...
    	sb.append("\t<a target=\"_blank\" name=\"google_share\" href=\"https://plus.google.com/u/1/share?url=" + HtmlUtil.encode(completeUrl) + "&source=campaignalyzer.com" +
    	...
    	sb.append("\t<a target=\"_blank\" name=\"linkedin_share\" href=\"http://www.linkedin.com/shareArticle?url=" + HtmlUtil.encode(completeUrl) + "&source=campaignalyzer.com" +
	...
	public static String renderTaggedUrlListForm(String myRole, Entity account, Entity campaign, long myLongTaggedUrlId,
	    ...
	    Iterable<Entity> taggedUrls = PMUtil.listChildren("TaggedUrl", campaign.getKey());
...
/*	    
	 // if user has permission AND is editing this tagged url
		...
		sb.append("<form method='post' action='/edittaggedurl?aid="+longAccountId+"&cid="+longCampaignId+"&act=" + Messages.ACTION_DELETEAll+"'>");
		
	    sb.append("<table class=\"table table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer modified\">\r\n");
	    sb.append("<thead><tr>\r\n");
	    ...
	    sb.append("\t<th class='no-sort' style='width:130px !important;'>Action</th>\r\n");
	    sb.append("</tr>\r\n");
	    sb.append("</thead>\r\n");
	    sb.append("<tbody>\r\n");
		...
		for (Entity taggedUrl : taggedUrls) {
	    	...
	    	utm_url = (String) taggedUrl.getProperty("utm_url");
	    	if (null==utm_url) utm_url = "";
	    	utm_term = (String) taggedUrl.getProperty("utm_term");
	    	if (null==utm_term) utm_term = "";
	    	utm_content = (String) taggedUrl.getProperty("utm_content");
	    	if (null==utm_content) utm_content = "";
	    	utm_extra = (String) taggedUrl.getProperty("utm_extra");
	    	if (null==utm_extra) utm_extra="";
	    	modifiedAt = (Date) taggedUrl.getProperty("modifiedAt");
	...
	/*private static String renderTaggedUrlFormRow(long longAccountId, long longCampaignId, long longTaggedUrlId, 
		    String utm_url, String utm_term, String utm_content, String utm_extra) {*/
	...
	public static String renderTaggedUrlFormRow(long longAccountId, long longCampaignId, Entity taggedUrl, long longTaggedUrlId) {
	    ...
	    String cName = (String) mycampaign.getProperty("utm_campaign");
		...
		if (0==longTaggedUrlId) {
		 ...
		 sb.append("<table class=\"table table table-striped table-bordered table-hover table-checkable order-column dataTable no-footer\">\r\n");
		    sb.append("<thead><tr>\r\n");
		    sb.append("\t<th style='display:none;'>Id</th>\r\n");
		    sb.append("<th class='no-sort' style='padding:8px 10px; width:50px !important;'><input type=\"checkbox\" id='deletAll'/></th>\r\n");
		    sb.append("\t<th>Destination URL*</th>\r\n");
		    sb.append("\t<th style='display:none;'>Shortned</th>\r\n");
		    sb.append("\t<th>Term</th>\r\n");
		    sb.append("\t<th>Content</th>\r\n");
		    sb.append("\t<th>Extra Parameter</th>\r\n");
		    sb.append("\t<th class=\"notSortable\">Action</th>\r\n");
		    sb.append("</tr>\r\n");
		    sb.append("</thead>\r\n");
		    sb.append("<tbody>\r\n");
		
    	sb.append("<form action=\"/edittaggedurl\" method=\"post\" name=\"taggedUrlForm\" onsubmit=\"return validateTaggedUrlForm()\"><tr class=\"urlForm\">");
	    //sb.append("<td>&nbsp;</td>\r\n");
    	sb.append("<td><input type=\"checkbox\" name=\"cid\" class='checkboxes' value=\"" + longCampaignId + "\" /></td>\r\n");
	    sb.append("<td><input class='form-control' name=\"utm_url\" type=\"text\" value=\"" + utm_url + "\" height=\"4\"/></td>\r\n");
	    ...
	    sb.append("<td><input class='form-control' name=\"utm_extra\" type=\"text\" value=\"" + utm_extra + "\" /></td>\r\n");
	    ...
	    sb.append("<input type=\"hidden\" name=\"cName\" value=\"" + cName + "\"/>");
	    sb.append("</form>");
	...
	public static String renderNewTaggedUrlFormRow(long longAccountId, long longCampaignId) {
    	...
    	sb.append("<form action=\"/edittaggedurl\" method=\"post\" name=\"taggedUrlForm\" onsubmit=\"return validateTaggedUrlForm()\"><tr class=\"urlForm\">");
	    //sb.append("<td>&nbsp;</td>\r\n");
	    sb.append("<td><input class='form-control' style='width:97%' name=\"utm_url\" type=\"text\" value='' height=\"4\"/></td>\r\n");
	    sb.append("<td><input class='form-control' style='width:97%' name=\"utm_term\" type=\"text\" value='' /></td>\r\n");
	    sb.append("<td><input class='form-control' style='width:97%' name=\"utm_content\" type=\"text\" value='' /></td>\r\n");
	    sb.append("<td><input class='form-control' style='width:97%' name=\"utm_extra\" type=\"text\" value='' /></td>\r\n");
	    sb.append("<td><input class='btn green' name=\"save\" type=\"submit\" value=\"" + buttonLabel + "\" /></td>\r\n");
	    sb.append("</tr>\r\n");
	    sb.append("<input type=\"hidden\" name=\"aid\" value=\"" + longAccountId +"\"/>");
	    sb.append("<input type=\"hidden\" name=\"cid\" value=\"" + longCampaignId +"\"/>");
	...
	public static String renderTaggedUrlDetailsRow(String myRole, long longAccountId, long longCampaignId, long longTaggedUrlId, 
		...
		sb.append("<td><a id="+longTaggedUrlId+" href='"+ utm_long_url +"' target='_blank' title='"+ utm_long_url+"'>" + utm_url + "</a><p></p>");
		...
		if (UserType.hasPermission(myRole, UserType.PERMISSION_MANAGE_TAGGED_URLS) ){
			sb.append("<a class='btn purple btn-outline' href=\"/ManageTaggedURLs.jsp?aid=" + longAccountId  + "&cid=" + longCampaignId  + "&tid=" + longTaggedUrlId +"&cname=" + cName + "\">Edit</a>");
			//sb.append("&nbsp;|&nbsp;");
			sb.append("<a class='btn red btn-outline' href=\"javascript:confirmTaggedUrlDelete('/edittaggedurl?aid=" + longAccountId  + "&cid=" + longCampaignId  + "&tid=" + longTaggedUrlId + "&act=" + Messages.ACTION_DELETE + "')\">Delete</a>");
			//sb.append("&nbsp;|&nbsp;");
	...
	public static String renderTaggedUrlExportList(Entity account, String[] sCampaignIds, String delimiter) {
		...
		if (null!=sCampaignIds) {
			for (int i=0; i<sCampaignIds.length; i++) {
				Iterable<Entity> cList = PMUtil.listEntities("Campaign", "utm_campaign", sCampaignIds[i]);
				for(Entity campaignl : cList){
					longCampaignId = campaignl.getKey().getId();
					if (0!=longCampaignId) {
						campaign = CampaignService.getSingleCampaign(longAccountId, longCampaignId);
			
						if (null!=campaign) {
							longCampaignId = campaign.getKey().getId();
							utm_campaign = (String) campaign.getProperty("utm_campaign");
							utm_medium = (String) campaign.getProperty("utm_medium");
							utm_source = (String) campaign.getProperty("utm_source");
	...
	public static String renderTaggedUrlExportListForMS(Entity account, String[] sCampaignIds, String delimiter) {
		StringBuilder sb = new StringBuilder();
		long longAccountId = account.getKey().getId();
