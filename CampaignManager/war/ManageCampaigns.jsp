...
<%
	...
	if ("".equals(myEmail)) {
%>
<% response.sendRedirect("/signin.jsp?continuer=ManageCampaign.jsp"); %>
...
<%
	// check permission
	} else if (!UserType.hasPermission(myRole, UserType.PERMISSION_VIEW_ACCOUNT) && !UserServiceFactory.getUserService().isUserAdmin()) {
%>
...
%>
<%= CampaignService.renderCampaignList(myRole, longAccountId, campaignName) %>
<%-- <%= CampaignHolderService.renderCampaignHolderList(myRole, longAccountId) %> --%>
<%
	}
%>
	</div>
